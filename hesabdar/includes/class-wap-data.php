...
class WAP_Data {
    ...
    public static function line_matches_products( $item, array $product_ids ): bool {
        ...
        // همسو با موجودی WC: ترجیح شناسه ورییشن/ساده از WAP_Stock
        if ( class_exists( 'WAP_Stock' ) ) {
            $stock_id = (int) WAP_Stock::id_from_order_item( $item );
            if ( $stock_id > 0 && in_array( $stock_id, $product_ids, true ) ) {
                return true;
            }
        }
    ...
    public static function get_orders( $f ) {
        ...
        $status_filter = self::wc_status_filter( is_array( $f ) ? $f : array() );
    ...
    public static function get_filtered_order_list( array $f ): array {
        ...
        if ( ! empty( $status_filter ) ) { $args['status'] = $status_filter; }
