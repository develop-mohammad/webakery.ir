...
$checks = array(
  ...
  $root.'/assets/style.css' => array('wap-peak-cal__grid','wap-peak-dayview__fill','wap-peak-weekcal__cell'),
...
// unit: empty calendar still builds current month
$cal = WAP_Analytics::peak_calendar(array(), array('date_from'=>'1405/06/01','date_to'=>'1405/06/31'));
if (empty($cal['months']) || $cal['months'][0]['m']!==6) { fwrite(STDERR,"FAIL month build\n"); var_export($cal); exit(1);} 
$days = array_filter($cal['months'][0]['days'], function($d){ return empty($d['blank']); });
if (count($days)!==31) { fwrite(STDERR,"FAIL day count ".count($days)."\n"); exit(1);} 
echo "$ht=WAP_Analytics::heat_tone(12,12);
if($ht['fg']!=='#ffffff' || $ht['level']!==5){ fwrite(STDERR,"FAIL heat_tone peak
"); exit(1);} 
$hl=WAP_Analytics::heat_tone(1,12);
if($hl['level']!==1 || $hl['fg']==='#ffffff'){ fwrite(STDERR,"FAIL heat_tone low
"); exit(1);} 
echo "OK heat_tone
";
echo "OK peak_calendar unit\nALL PEAK CALENDAR CHECKS PASSED\n";
