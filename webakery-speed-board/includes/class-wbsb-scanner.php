...
class WBSB_Scanner {
	...
	public static function scan( $url = '' ) {
		...
		$resp = wp_remote_get(
			...
			array(
				...
				'headers' => array(
					'Accept'     => 'text/html',
					'User-Agent' => 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) WebakerySpeedBoard/' . WBSB_VERSION,
				),
			)
		);
	...
	private static function step_search_console() {
		...
		return array(
			...
			'issues'   => $issues,
			'actions'  => array(
				'لینک GSC یا PSI را در بخش بالا ذخیره کن.',
				'مقادیر LCP / INP / CLS فیلد دیتا را وارد کن.',
	...
	private static function step_fonts( $html ) {
		...
		return array(
			...
			'actions' => array(
				'پلاگین فونت سوییپ 1.1 را نصب/آپدیت کن',
				'حذف Google Fonts + حذف Preloadهای مضر + فقط woff2',
				'حداکثر Preload را 2 یا 3 بگذار',
			),
		);
	}
