		...
		foreach ( $enriched as $row ) {
			...
			$lines[]  = implode(
				...
				array(
					...
					self::csv_escape( isset( $row['entity_fa'] ) ? $row['entity_fa'] : ( isset( $row['entity'] ) ? $row['entity'] : '' ) ),
					...
					! empty( $row['iran_trend'] ) ? '1' : '0',
					self::csv_escape( isset( $row['trend_traffic'] ) ? $row['trend_traffic'] : '' ),
