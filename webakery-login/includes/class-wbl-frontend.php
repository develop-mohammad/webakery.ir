...
class WBL_Frontend {
	...
	public static function hooks() {
		...
		add_action( 'woocommerce_before_customer_login_form', array( __CLASS__, 'wc_login_inject' ) );
	}
	...
	public static function shortcode( $atts = array() ) {
		...
		if ( is_user_logged_in() ) {
			...
			<div class="wbl-box wbl-logged">
				...
				<p class="wbl-actions">
					<a class="wbl-btn wbl-btn-primary" href="<?php echo esc_url( WBL_Auth::redirect_url() ); ?>">ورود به حساب</a>
		...
		$error       = isset( $_GET['wbl_error'] ) ? sanitize_text_field( wp_unslash( $_GET['wbl_error'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		$google_url  = $show_google ? WBL_Google::auth_url( $redirect ) : '';
	...
	public static function wc_login_inject() {
		if ( ! WBL_Plugin::is_usable() ) {
			return;
		}
	...
	public static function maybe_replace_wp_login() {
		...
		if ( ! empty( $_REQUEST['interim-login'] ) || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			return;
		}
