...
class WBL_Settings {
	...
	public static function save( array $input ) {
		...
		$ints = array( 'otp_length', 'otp_ttl', 'otp_resend_wait', 'otp_max_attempts', 'otp_daily_limit', 'otp_ip_hourly_limit', 'login_page_id' );
		foreach ( $ints as $k ) {
			if ( array_key_exists( $k, $input ) ) {
		...
		$out['otp_ip_hourly_limit'] = min( 100, max( 5, (int) $out['otp_ip_hourly_limit'] ) );
