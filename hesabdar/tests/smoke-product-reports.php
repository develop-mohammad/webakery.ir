...
# Port presence checks (unified plugin)
$boot = file_get_contents( dirname( __DIR__ ) . '/hesabdar.php' );
...
assert( strpos( $boot, '1.25.0' ) !== false, 'version 1.25.0' );
assert( strpos( $boot, 'class-wci-tracking.php' ) !== false, 'tracking loaded' );
assert( strpos( $portal, 'render_buyers_tab' ) !== false, 'buyers tab' );
assert( strpos( $portal, "wap_view', 'buyers'" ) !== false || strpos( $portal, "'buyers'" ) !== false, 'buyers view' );
assert( strpos( $export, 'function buyers_csv' ) !== false, 'buyers_csv' );
assert( strpos( file_get_contents( dirname( __DIR__ ) . '/includes/class-wap-data.php' ), 'line_matches_products' ) !== false, 'line_matches' );
