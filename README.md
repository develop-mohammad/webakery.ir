...
- **فاکتورها** در کنار سفارش‌ها (ایجاد از سفارش، چاپ/PDF، CSV)
- نقش‌های **مدیر فروشگاه** و **حسابدار** با منوی «پنل فروشگاه»
- ذخیره پیش‌نویس فرم روی لپ‌تاپ/مرورگر (`localStorage`) + دانلود فایل JSON
- دانلود CSV سفارش‌ها و فاکتورها از پیشخوان
- استایل فرانت RTL و فارسی

### فاکتور و پنل حسابدار / مدیر

1. پلاگین را به نسخه `1.1.0` به‌روزرسانی یا دوباره فعال کنید.
2. از **کاربران → افزودن** نقش `مدیر فروشگاه` یا `حسابدار` بدهید.
3. در منوی **پنل فروشگاه** یا **Webakery → فاکتورها** فاکتور بسازید.
4. از لیست/جزئیات هر سفارش دکمه **ایجاد فاکتور** را بزنید.
