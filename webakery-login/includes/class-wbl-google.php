...
class WBL_Google {
	...
	/**
	 * @param string $return_url آدرس برگشت پس از ورود موفق.
	 */
	public static function auth_url( $return_url = '' ) {
		if ( ! self::enabled() ) {
			return '';
		}
		...
		set_transient(
			...
			array(
				'return' => $return_url ? esc_url_raw( $return_url ) : '',
			),
	...
	public static function maybe_handle_callback() {
		...
		if ( ! WBL_Plugin::is_usable() ) {
			wp_safe_redirect( add_query_arg( 'wbl_error', rawurlencode( 'لایسنس افزونه فعال نیست.' ), self::login_page_url() ) );
			exit;
		}
		...
		$saved = $state ? get_transient( self::STATE_TRANSIENT . $state ) : false;
		...
		$return = is_array( $saved ) ? ( $saved['return'] ?? '' ) : '';
		...
		$dest = WBL_Auth::redirect_url( $return );
		wp_safe_redirect( $dest ?: $result['redirect'] );
		exit;
	}
	...
	private static function exchange_code( $code ) {
		...
		if ( ! $token ) {
			$msg = is_array( $data ) && ! empty( $data['error_description'] ) ? $data['error_description'] : 'توکن دسترسی از گوگل دریافت نشد.';
			return new WP_Error( 'wbl_google_token', $msg );
		}
		return $token;
	}
