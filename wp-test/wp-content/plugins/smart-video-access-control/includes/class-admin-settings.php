...
final class SVAC_Admin_Settings {
	...
	public static function render(): void {
		...
		<div class="wrap svac-admin" dir="rtl">
			...
			<section id="svac-rules" hidden><h2><?php esc_html_e( 'قوانین نقش برای دسته‌ها', 'smart-video-access-control' ); ?></h2><p><?php esc_html_e( 'وقتی برای خود ویدیو نقشی انتخاب نشده باشد، نقش‌های اینجا اعمال می‌شوند.', 'smart-video-access-control' ); ?></p>
				...
				<table class="widefat striped"><thead><tr><th><?php esc_html_e( 'دسته', 'smart-video-access-control' ); ?></th><th><?php esc_html_e( 'نقش‌های مجاز', 'smart-video-access-control' ); ?></th></tr></thead><tbody>
				<?php
				foreach ( $terms as $term ) :
					$selected = (array) get_term_meta( $term->term_id, '_svac_allowed_roles', true );
					?>
					<tr><td><?php echo esc_html( $term->name ); ?></td><td><select name="roles[<?php echo esc_attr( (string) $term->term_id ); ?>][]" multiple size="4">
					<?php
					foreach ( $roles as $key => $role ) :
						?>
					<option value="<?php echo esc_attr( $key ); ?>" <?php selected( in_array( $key, $selected, true ) ); ?>><?php echo esc_html( translate_user_role( $role['name'] ) ); ?></option><?php endforeach; ?></select></td></tr><?php endforeach; ?></tbody></table><?php submit_button( __( 'ذخیره قوانین دسته‌ها', 'smart-video-access-control' ) ); ?></form></section>
			<section id="svac-logs" hidden><h2><?php esc_html_e( 'گزارش دسترسی اخیر', 'smart-video-access-control' ); ?></h2><p><a class="button button-secondary" href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin-post.php?action=svac_export_logs' ), 'svac_export_logs' ) ); ?>"><?php esc_html_e( 'دریافت CSV', 'smart-video-access-control' ); ?></a></p><table class="widefat striped"><thead><tr><th>ID</th><th><?php esc_html_e( 'کاربر', 'smart-video-access-control' ); ?></th><th><?php esc_html_e( 'ویدیو', 'smart-video-access-control' ); ?></th><th><?php esc_html_e( 'زمان', 'smart-video-access-control' ); ?></th><th>IP</th><th><?php esc_html_e( 'وضعیت', 'smart-video-access-control' ); ?></th></tr></thead><tbody>
			<?php
			foreach ( SVAC_Access_Logs::get_logs() as $log ) :
				$log_user  = $log['user_id'] ? get_userdata( (int) $log['user_id'] ) : false;
				$user_name = $log_user ? $log_user->display_name : __( 'مهمان', 'smart-video-access-control' );
				$video_ttl = get_the_title( (int) $log['video_id'] );
				if ( '' === $video_ttl ) {
					$video_ttl = '#' . $log['video_id'];
				}
				?>
				<tr><td><?php echo esc_html( $log['id'] ); ?></td><td><?php echo esc_html( $user_name ); ?></td><td><?php echo esc_html( $video_ttl ); ?></td><td><?php echo esc_html( get_date_from_gmt( $log['access_time'], get_option( 'date_format' ) . ' ' . get_option( 'time_format' ) ) ); ?></td><td><?php echo esc_html( $log['ip_address'] ); ?></td><td><?php echo esc_html( $log['status'] ); ?></td></tr><?php endforeach; ?></tbody></table></section>
		</div>
		<?php
	}
	...
	public static function save_category_rules(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'اجازه کافی ندارید.', 'smart-video-access-control' ) );
		}
		check_admin_referer( 'svac_save_category_rules' );
		$roles = isset( $_POST['roles'] ) ? (array) map_deep( wp_unslash( $_POST['roles'] ), 'sanitize_key' ) : array();
	...
	public static function export_logs(): void {
		...
		foreach ( SVAC_Access_Logs::get_logs( 500 ) as $log ) {
			fputcsv( $output, $log ); }
		fclose( $output );
		exit;
	}
