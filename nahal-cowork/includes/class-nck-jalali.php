...
/**
 * تبدیل تقویم شمسی/میلادی — مستقل از وردپرس.
...
class NCK_Jalali {
	...
	public static function parse( $str ) {
		$str = NCK_Phone::latin_digits( (string) $str );
		$str = trim( $str );
	...
	public static function format_long( $jy, $jm, $jd ) {
		$names = self::month_names();
		$jm    = (int) $jm;
		$name  = isset( $names[ $jm - 1 ] ) ? $names[ $jm - 1 ] : '';
		return self::fa_digits( (int) $jd . ' ' . $name . ' ' . (int) $jy );
	}

	public static function month_range_g( $jy, $jm ) {
		$len   = self::month_length( $jy, $jm );
		$start = self::to_g_date( $jy, $jm, 1 );
		$end   = self::to_g_date( $jy, $jm, $len );
