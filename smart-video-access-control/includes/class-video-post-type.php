...
final class SVAC_Video_Post_Type {
	...
	public static function register(): void {
		...
		register_taxonomy(
			...
			'svac_video',
			array(
				'labels'            => array( 'name' => __( 'دسته‌های ویدیو', 'smart-video-access-control' ), 'singular_name' => __( 'دسته ویدیو', 'smart-video-access-control' ) ),
				'public'            => false,
				'show_ui'           => true,
	...
	public static function render_meta_box( WP_Post $post ): void {
		...
		<tr><th><label for="svac_allowed_roles"><?php esc_html_e( 'نقش‌های مجاز', 'smart-video-access-control' ); ?></label></th><td><select id="svac_allowed_roles" name="svac_allowed_roles[]" multiple size="5"><?php foreach ( $roles as $key => $role ) : ?><option value="<?php echo esc_attr( $key ); ?>" <?php selected( in_array( $key, $meta['allowed_roles'], true ) ); ?>><?php echo esc_html( translate_user_role( $role['name'] ) ); ?></option><?php endforeach; ?></select><p class="description"><?php esc_html_e( 'خالی یعنی نقش محدودیتی ندارد.', 'smart-video-access-control' ); ?></p></td></tr>
		<tr><th><label for="svac_allowed_users"><?php esc_html_e( 'کاربران مجاز', 'smart-video-access-control' ); ?></label></th><td><select id="svac_allowed_users" name="svac_allowed_users[]" multiple size="5"><?php foreach ( $users as $user ) : ?><option value="<?php echo esc_attr( $user->ID ); ?>" <?php selected( in_array( (int) $user->ID, $meta['allowed_users'], true ) ); ?>><?php echo esc_html( $user->display_name . ' (' . $user->user_login . ')' ); ?></option><?php endforeach; ?></select><p class="description"><?php esc_html_e( 'کاربران مجاز بر محدودیت نقش غلبه می‌کنند.', 'smart-video-access-control' ); ?></p></td></tr>
		<tr><th><label for="svac_blocked_users"><?php esc_html_e( 'کاربران مسدود', 'smart-video-access-control' ); ?></label></th><td><select id="svac_blocked_users" name="svac_blocked_users[]" multiple size="5"><?php foreach ( $users as $user ) : ?><option value="<?php echo esc_attr( $user->ID ); ?>" <?php selected( in_array( (int) $user->ID, $meta['blocked_users'], true ) ); ?>><?php echo esc_html( $user->display_name . ' (' . $user->user_login . ')' ); ?></option><?php endforeach; ?></select></td></tr>
	...
	public static function save_meta( int $post_id ): void {
		...
		foreach ( array( 'allowed_users', 'blocked_users' ) as $field ) {
			$values = isset( $_POST[ 'svac_' . $field ] ) ? (array) wp_unslash( $_POST[ 'svac_' . $field ] ) : array();
			update_post_meta( $post_id, '_svac_' . $field, array_values( array_unique( array_filter( array_map( 'absint', $values ) ) ) ) );
		}
		$roles = isset( $_POST['svac_allowed_roles'] ) ? (array) wp_unslash( $_POST['svac_allowed_roles'] ) : array();
	...
	public static function get_meta( int $video_id ): array {
		return array(
			...
			'allowed_users' => array_map( 'absint', (array) get_post_meta( $video_id, '_svac_allowed_users', true ) ),
			'blocked_users' => array_map( 'absint', (array) get_post_meta( $video_id, '_svac_blocked_users', true ) ),
			'allowed_roles' => array_map( 'sanitize_key', (array) get_post_meta( $video_id, '_svac_allowed_roles', true ) ),
