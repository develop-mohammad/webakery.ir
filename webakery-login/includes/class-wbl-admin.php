...
class WBL_Admin {
	...
	public static function handle_save() {
		if ( empty( $_POST['wbl_save_settings'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			return;
		}
		...
		$tab   = isset( $_POST['wbl_tab'] ) ? sanitize_key( wp_unslash( $_POST['wbl_tab'] ) ) : 'general'; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$input = isset( $_POST['settings'] ) && is_array( $_POST['settings'] ) ? wp_unslash( $_POST['settings'] ) : array(); // phpcs:ignore

		// فقط چک‌باکس‌های همان تب را صفر کن؛ وگرنه تنظیمات تب‌های دیگر پاک می‌شود.
		$tab_bools = array(
	...
	public static function save_user_phone( $user_id ) {
		...
		if ( ! isset( $_POST['wbl_phone'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			return;
		}
		$raw = wp_unslash( $_POST['wbl_phone'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
		if ( '' === trim( (string) $raw ) ) {
