...
class WBSB_Plugin {
	...
	private function __construct() {
		...
		add_filter( 'plugin_action_links_' . plugin_basename( WBSB_FILE ), array( $this, 'links' ) );
	}
	...
	public function assets( $hook ) {
		if ( 'toplevel_page_webakery-speed-board' !== $hook ) {
			return;
		}
		wp_enqueue_style( 'wbsb-admin', WBSB_URL . 'assets/admin.css', array(), WBSB_VERSION );
	}

	public function links( $links ) {
		array_unshift( $links, '<a href="' . esc_url( admin_url( 'admin.php?page=webakery-speed-board' ) ) . '"><strong>باز کردن پنل</strong></a>' );
		return $links;
	}
	...
	public function save_gsc() {
		...
		wp_safe_redirect( admin_url( 'admin.php?page=webakery-speed-board&saved=1' ) );
		exit;
	}
	...
	public function run_scan() {
		...
		$result = WBSB_Scanner::scan( home_url( '/' ) );
		...
		wp_safe_redirect( admin_url( 'admin.php?page=webakery-speed-board&scanned=1' ) );
		exit;
	}
	...
	public function toggle_done() {
		...
		wp_safe_redirect( admin_url( 'admin.php?page=webakery-speed-board' ) );
		exit;
	}
	...
	public function render() {
		...
		<div class="wrap wbsb-wrap">
			<div class="wbsb-hero">
				...
				<span class="wbsb-badge">v<?php echo esc_html( WBSB_VERSION ); ?></span>
			</div>
